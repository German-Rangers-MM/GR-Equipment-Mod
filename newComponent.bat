...
:author
set /p "author=Enter Authors name: "
if "%author%"=="" (
...
:: Prompt for stringtable
choice /m "Is a Stringtable required?"
set "stringtable=%ERRORLEVEL%"

:: Prompt for Data Folder
choice /m "Do you want to create a Data folder? (for textures, models, etc.)"
set "dataFolder=%ERRORLEVEL%"
...
if %dataFolder% equ 1 (
	...
	if not exist "%dataPath%" (
		echo Error: Failed to create folder "%dataPath%"
		pause
		exit /b 1
	)
)
...
echo $content = $content -replace 'AUTHORSPLACEHOLDER', "`"$author`""
...
powershell -ExecutionPolicy Bypass -File "%tempPs%" "%tempFile%" "%folderPath%\script_component.hpp" "%Component%" "%ComponentBeautified%" "%ComponentUpperCase%" "%relativepath%" "%relativepathTrimmed%" "%author%"
del "%tempFile%"

if %stringtable% equ 1 (
	set "tempFile=%TEMP%\stringtable"
	copy /Y "%templatePath%\stringtable.xml" "%tempFile%" > nul
	powershell -ExecutionPolicy Bypass -File "%tempPs%" "%tempFile%" "%folderPath%\stringtable.xml" "%Component%" "%ComponentBeautified%" "%ComponentUpperCase%" "%relativepath%" "%relativepathTrimmed%" "%author%"
	del "%tempFile%"
)
:: Create PBOPREFIX
...
if %cbaXEH% equ 1 (
	...
	for %%F in (CfgEventHandlers.hpp XEH_preStart.sqf XEH_preInit.sqf XEH_postInit.sqf XEH_PREP.hpp) do (
		...
		powershell -ExecutionPolicy Bypass -File "%tempPs%" "%tempFile%" "%folderPath%\%%F" "%Component%" "%ComponentBeautified%" "%ComponentUpperCase%" "%relativepath%" "%relativepathTrimmed%" "%author%"
		del "%tempFile%"
	)
	...
	powershell -ExecutionPolicy Bypass -File "%tempPs%" "%tempFile%" "%folderPath%\config.cpp" "%Component%" "%ComponentBeautified%" "%ComponentUpperCase%" "%relativepath%" "%relativepathTrimmed%" "%author%"
	del "%tempFile%"
) else (
...
if !cfgSounds! equ 1 (
	...
	if not exist "%folderPath%\CfgSounds.hpp" (
	   (
	   	echo class CfgSounds {
	   	echo };
	   ) > "%folderPath%\CfgSounds.hpp"
	   (
		  echo:
		  echo #include "CfgSounds.hpp"
	   ) >> "%folderPath%\config.cpp"
	   if exist "%folderPath%\CfgSounds.hpp" (
		  echo Successfully created CfgSounds.hpp in "%folderPath%"
	   ) else (
		  echo Error: Failed to create CfgSounds.hpp in "%folderPath%"
	   )
	) else (
	   echo CfgSounds.hpp already exists in "%folderPath%"
	)
)

if !cfgSoundSets! equ 1 (
	if !exitCall! equ 1 (
		goto menuError
	)
	if not exist "%folderPath%\CfgSoundSets.hpp" (
	   (
	   	echo class CfgSoundSets {
	   	echo };
	   ) > "%folderPath%\CfgSoundSets.hpp"
	   (
		  echo:
		  echo #include "CfgSoundSets.hpp"
	   ) >> "%folderPath%\config.cpp"
	   if exist "%folderPath%\CfgSoundSets.hpp" (
		  echo Successfully created CfgSoundSets.hpp in "%folderPath%"
	   ) else (
		  echo Error: Failed to create CfgSoundSets.hpp in "%folderPath%"
	   )
	) else (
	   echo CfgSoundSets.hpp already exists in "%folderPath%"
	)
)

if !cfgSoundShaders! equ 1 (
	if !exitCall! equ 1 (
		goto menuError
	)
	if not exist "%folderPath%\CfgSoundShaders.hpp" (
	   (
	   	echo class CfgSoundShaders {
	   	echo };
	   ) > "%folderPath%\CfgSoundShaders.hpp"
	   (
		  echo:
		  echo #include "CfgSoundShaders.hpp"
	   ) >> "%folderPath%\config.cpp"
	   if exist "%folderPath%\CfgSoundShaders.hpp" (
		  echo Successfully created CfgSoundShaders.hpp in "%folderPath%"
	   ) else (
		  echo Error: Failed to create CfgSoundShaders.hpp in "%folderPath%"
	   )
	) else (
	   echo CfgSoundShaders.hpp already exists in "%folderPath%"
	)
)
